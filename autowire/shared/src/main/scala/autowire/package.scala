...
import autowire.Internal.ClientCallable
...
package object autowire extends autowire.Internal.LowPri {
   ...
   * that takes in [[autowire.Core.Request]] objects and spits out serialized
   * `Future[String]`s in response.
   *
   * It is not concerned with how the [[autowire.Core.Request]] objects get to it, or
   * how the marshalled `Future[String]` will be transmitted back to the
   * client: it simply calls the function described by the [[autowire.Core.Request]]
  ...
  implicit def clientFutureCallable[T](t: Future[T]): ClientCallable[T] = new Internal.ClientCallable[T]
  /**
